...
df['share_innov_pat_without'] = sol_baseline.psi_m_star[...,1].ravel()**-p_baseline.k[1]
df['share_innov_pat_with'] = sol_baseline.psi_m_star_with_prod_patent[...,1].ravel()**-p_baseline.k[1]
...
#%% Patents flows - trade flows regressions

# gravity_patents_output_path

data_left = pd.read_csv(gravity_patents_path+'pat_on_dist_group_avgs.csv')
...
#%% Write the ratio of patent preparation and filing costs to RD costs

patent_prep_costs = sol_baseline.w*sol_baseline.l_Ao[:,1]
patent_filing_costs = np.einsum('n,in->i',sol_baseline.w,sol_baseline.l_Ae[:,:,1])
total_RD_costs = sol_baseline.w*sol_baseline.l_R[:,1] + sol_baseline.w*sol_baseline.l_Ao[:,1] + np.einsum('n,in->i',sol_baseline.w,sol_baseline.l_Ae[:,:,1])

df = pd.DataFrame(index=p_baseline.countries,columns=['patent_prep_costs','patent_filing_costs','total_RD_costs'])
df['patent_prep_costs'] = patent_prep_costs
df['patent_filing_costs'] = patent_filing_costs
df['total_RD_costs'] = total_RD_costs
...
p_multi = parameters()
p_multi.load_run(multi_sector_run_path)


df = pd.DataFrame(
    index = pd.Index(
                    [countries_names[c] for c in p_baseline.countries]
                    ,name='Countries'
                    ),
    columns = ['delta','eta','nu'])

df.loc[[countries_names[c] for c in p_baseline.countries],'delta'] = p_multi.delta[:,1]
df.loc[[countries_names[c] for c in p_baseline.countries],'eta'] = p_multi.eta[:,1]
df.loc[[countries_names[c] for c in p_baseline.countries],'nu'] = p_multi.nu[1]
    
df.to_csv(multi_sector_path+'calibration_patenting.csv',float_format='%.6f')
...
df.loc[[countries_names[c] for c in p_baseline.countries],'delta'] = p_multi.delta[:,2]
df.loc[[countries_names[c] for c in p_baseline.countries],'eta'] = p_multi.eta[:,2]
df.loc[[countries_names[c] for c in p_baseline.countries],'nu'] = p_multi.nu[2]
    
df.to_csv(multi_sector_path+'calibration_pharmachem.csv',float_format='%.6f')

m_multi = moments()
# m_multi.load_data()
m_multi.load_run(multi_sector_run_path)

sol_multi = var.var_from_vector(p_multi.guess, p_multi, compute=True, context = 'counterfactual')
sol_multi.scale_P(p_multi)
sol_multi.compute_non_solver_quantities(p_multi)
...
df_summary['change in real profits (%)'] = (np.einsum('in,i,n->n',dyn_sol_nash.sol_fin.profit[...,1],dyn_sol_nash.sol_fin.w,1/dyn_sol_nash.sol_fin.price_indices))\
    /(np.einsum('in,i,n->n',dyn_sol_nash.sol_init.profit[...,1],dyn_sol_nash.sol_init.w,1/dyn_sol_nash.sol_init.price_indices))-1
...
df_summary['change in real profits (%)'] = (np.einsum('in,i,n->n',dyn_sol_coop_equal.sol_fin.profit[...,1],dyn_sol_coop_equal.sol_fin.w,1/dyn_sol_coop_equal.sol_fin.price_indices))\
    /(np.einsum('in,i,n->n',dyn_sol_coop_equal.sol_init.profit[...,1],dyn_sol_coop_equal.sol_init.w,1/dyn_sol_coop_equal.sol_init.price_indices))-1
...
#%% Partial equilibriums Nash

sol, sol_init = fixed_point_solver(p_baseline,x0=p_baseline.guess,
                                context = 'counterfactual',
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
...
sol_init.scale_P(p_baseline)
sol_init.compute_non_solver_quantities(p_baseline)
...
sol, dyn_sol = dyn_fixed_point_solver(p_nash, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
sol, dyn_sol_exog_lr = dyn_fixed_point_solver_exog_lr(p_nash,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_lr.compute_non_solver_quantities(p_nash)

sol, dyn_sol_exog_patent = dyn_fixed_point_solver_exog_patent_thresholds(p_nash,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_patent.compute_non_solver_quantities(p_nash)

sol, dyn_sol_exog_lr_and_patent = dyn_fixed_point_solver_exog_lr_and_patent_thresholds(p_nash,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_lr_and_patent.compute_non_solver_quantities(p_nash)

df = pd.DataFrame(index=p_baseline.countries+['Equal','Negishi'])

df['full solve'] = dyn_sol.cons_eq_welfare.tolist() + [dyn_sol.cons_eq_pop_average_welfare_change] + [dyn_sol.cons_eq_negishi_welfare_change]
df['exog l_R'] = dyn_sol_exog_lr.cons_eq_welfare.tolist() + [dyn_sol_exog_lr.cons_eq_pop_average_welfare_change] + [dyn_sol_exog_lr.cons_eq_negishi_welfare_change]
df['exog patent thresh'] = dyn_sol_exog_patent.cons_eq_welfare.tolist() + [dyn_sol_exog_patent.cons_eq_pop_average_welfare_change] + [dyn_sol_exog_patent.cons_eq_negishi_welfare_change]
df['exog l_R and patent thresh'] = dyn_sol_exog_lr_and_patent.cons_eq_welfare.tolist() + [dyn_sol_exog_lr_and_patent.cons_eq_pop_average_welfare_change] + [dyn_sol_exog_lr_and_patent.cons_eq_negishi_welfare_change]

df.to_csv(partial_equilibria_path+'nash.csv')

#%% Partial equilibriums Coop Equal
...
sol, dyn_sol = dyn_fixed_point_solver(p_coop_equal, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
sol, dyn_sol_exog_lr = dyn_fixed_point_solver_exog_lr(p_coop_equal,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_lr.compute_non_solver_quantities(p_coop_equal)

sol, dyn_sol_exog_patent = dyn_fixed_point_solver_exog_patent_thresholds(p_coop_equal,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_patent.compute_non_solver_quantities(p_coop_equal)

sol, dyn_sol_exog_lr_and_patent = dyn_fixed_point_solver_exog_lr_and_patent_thresholds(p_coop_equal,p_baseline, sol_init, Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_exog_lr_and_patent.compute_non_solver_quantities(p_coop_equal)
...
df.to_csv(partial_equilibria_path+'coop_equal.csv')
...
#%% Write excel spreadsheets for variation with no obsolescence

from data_funcs import write_calibration_results

p_pre = parameters()
p_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline}_variations/{variation_with_no_obsolescence}/')
_, sol_pre = fixed_point_solver(p_pre,context = 'counterfactual',x0=p_pre.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
...
sol_pre.compute_non_solver_quantities(p_pre)
m_pre = moments()
m_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline}_variations/{variation_with_no_obsolescence}/')
m_pre.compute_moments(sol_pre,p_pre)
m_pre.compute_moments_deviations()

write_calibration_results(no_obsolescence_path+'no_obsolescence',p_pre,m_pre,sol_pre,commentary = '')


#%% Nash table with transitional dynamics with no obsolescence

all_nashes = pd.read_csv('nash_eq_recaps/dyn_deltas.csv')
all_nashes = all_nashes.drop_duplicates(['baseline','variation'],keep='last')

run_nash= all_nashes.loc[(all_nashes.baseline == int(baseline)) 
                         & (all_nashes.variation.astype(str) == variation_with_no_obsolescence)]

p_pre = parameters()
p_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline_pre_trips_variation}_variations/{variation_with_no_obsolescence}/')
_, sol_pre = fixed_point_solver(p_pre,context = 'counterfactual',x0=p_pre.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
...
m_pre = moments()
m_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline_pre_trips_variation}_variations/{variation_with_no_obsolescence}/')
...
caption = 'Nash equilibrium with no obsolescence'

df.style.format(precision=5).to_latex(no_obsolescence_path+'dyn_Nash_table_with_no_obsolescence.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(no_obsolescence_path+'dyn_Nash_table_with_no_obsolescence.csv',float_format='%.5f')

write_calibration_results(no_obsolescence_path+'dyn_Nash_with_no_obsolescence',p_nash,m_nash,dyn_sol_nash.sol_fin,commentary = '')


#%% Coop equal weights table with transitional dynamics with no obsolescence
...
run_coop_equal= all_coop_equales.loc[(all_coop_equales.baseline == int(baseline))
                                     & (all_coop_equales.variation == variation_with_no_obsolescence)
                                     & (all_coop_equales.aggregation_method == 'pop_weighted')]
...
df.style.format(precision=5).to_latex(no_obsolescence_path+'dyn_Coop_population_weights_table_with_no_obsolescence.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(no_obsolescence_path+'dyn_Coop_population_weights_table_with_no_obsolescence.csv',float_format='%.5f')

write_calibration_results(no_obsolescence_path+'dyn_Coop_population_weights_with_no_obsolescence',p_coop_equal,m_coop_equal,dyn_sol_coop_equal.sol_fin,commentary = '')

#%% Coop negishi weights table with transitional dynamics with no obsolescence
...
df.style.format(precision=5).to_latex(no_obsolescence_path+'dyn_Coop_negishi_weights_table_with_no_obsolescence.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(no_obsolescence_path+'dyn_Coop_negishi_weights_table_with_no_obsolescence.csv',float_format='%.5f')
...
sol_baseline_multi = var.var_from_vector(p_multi.guess, p_multi, compute=True, context = 'counterfactual')
sol_baseline_multi.scale_P(p_multi)
sol_baseline_multi.compute_non_solver_quantities(p_multi)

multi_sector_nash_path = f'coop_eq_direct_saves/dyn_{baseline}_{multi_sector_variation}_nash/'
p_nash = parameters()
p_nash.load_run(multi_sector_nash_path)

sol, dyn_sol_nash = dyn_fixed_point_solver(p_nash, sol_init=sol_baseline_multi,Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_nash.compute_non_solver_quantities(p_nash)
dyn_sol_nash.sol_fin.compute_consumption_equivalent_welfare(p_nash,sol_baseline_multi)
dyn_sol_nash.sol_fin.compute_world_welfare_changes(p_nash,sol_baseline_multi)
...
df = pd.DataFrame(index = pd.Index([countries_names[c] for c in p_baseline.countries]\
                                   +['World aggregate according to Negishi weights',
                                     ...
                                     'Growth rate (%)'],
                                   name = 'Countries'),
                  columns = [r'$\delta pharma/chem$',r'$\delta patenting$','Welfare change with transition dynamics',
                             'Welfare change, steady state only']
                  )
    
for i,c in enumerate(p_baseline.countries):
    df.loc[countries_names[c],r'$\delta patenting$'] = p_nash.delta[i,1]
    df.loc[countries_names[c],r'$\delta pharma/chem$'] = p_nash.delta[i,2]
    df.loc[countries_names[c],'Welfare change with transition dynamics'] = dyn_sol_nash.cons_eq_welfare[i]
    df.loc[countries_names[c],'Welfare change, steady state only'] = dyn_sol_nash.sol_fin.cons_eq_welfare[i]
...
# if policy was the same as monosector
...
p_nash.delta[:,1] = run_nash[p_baseline.countries].values.squeeze()
p_nash.delta[:,2] = run_nash[p_baseline.countries].values.squeeze()
...
df.to_csv(multi_sector_path+'dyn_Nash_table_if_same_policy_as_monosector.csv',float_format='%.5f')
...
multi_sector_equal_path = f'coop_eq_direct_saves/dyn_{baseline}_{multi_sector_variation}_pop_weighted/'
p_equal = parameters()
p_equal.load_run(multi_sector_equal_path)

sol, dyn_sol_equal = dyn_fixed_point_solver(p_equal, sol_init=sol_baseline_multi,Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_equal.sol_fin.compute_consumption_equivalent_welfare(p_equal,sol_baseline_multi)
dyn_sol_equal.sol_fin.compute_world_welfare_changes(p_equal,sol_baseline_multi)
...
for i,c in enumerate(p_baseline.countries):
    df.loc[countries_names[c],r'$\delta patenting$'] = p_equal.delta[i,1]
    df.loc[countries_names[c],r'$\delta pharma/chem$'] = p_equal.delta[i,2]
...
p_equal.delta[:,1] = run_coop_equal[p_baseline.countries].values.squeeze()
p_equal.delta[:,2] = run_coop_equal[p_baseline.countries].values.squeeze()
...
df.to_csv(multi_sector_path+'dyn_Coop_equal_table_if_same_policy_as_monosector.csv',float_format='%.5f')
...
multi_sector_negishi_path = f'coop_eq_direct_saves/dyn_{baseline}_{multi_sector_variation}_negishi/'
p_negishi = parameters()
p_negishi.load_run(multi_sector_negishi_path)

sol, dyn_sol_negishi = dyn_fixed_point_solver(p_negishi, sol_init=sol_baseline_multi,Nt=25,
                                      t_inf=500,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=False,
...
dyn_sol_negishi.compute_non_solver_quantities(p_negishi)
dyn_sol_negishi.sol_fin.compute_consumption_equivalent_welfare(p_negishi,sol_baseline_multi)
dyn_sol_negishi.sol_fin.compute_world_welfare_changes(p_negishi,sol_baseline_multi)
...
for i,c in enumerate(p_baseline.countries):
    df.loc[countries_names[c],r'$\delta patenting$'] = p_negishi.delta[i,1]
    df.loc[countries_names[c],r'$\delta pharma/chem$'] = p_negishi.delta[i,2]
    df.loc[countries_names[c],'Welfare change with transition dynamics'] = dyn_sol_negishi.cons_eq_welfare[i]
    df.loc[countries_names[c],'Welfare change, steady state only'] = dyn_sol_negishi.sol_fin.cons_eq_welfare[i]

df.loc['World aggregate according to Negishi weights',
       'Welfare change with transition dynamics'] = dyn_sol_negishi.cons_eq_negishi_welfare_change

df.loc['World aggregate according to Negishi weights',
       'Welfare change, steady state only'] = dyn_sol_negishi.sol_fin.cons_eq_negishi_welfare_change

df.loc['World aggregate according to population weights',
       'Welfare change with transition dynamics'] = dyn_sol_negishi.cons_eq_pop_average_welfare_change

df.loc['World aggregate according to population weights',
       'Welfare change, steady state only'] = dyn_sol_negishi.sol_fin.cons_eq_pop_average_welfare_change

df.loc['Growth rate (%)',
       'Welfare change, steady state only'] = dyn_sol_negishi.sol_fin.g*100

for col in df.columns:
    df[col] = df[col].astype(float)

df.to_csv(multi_sector_path+'dyn_Coop_negishi_table.csv',float_format='%.5f')
...
for qty in ['eta']:
    for c,country in enumerate(p_baseline.countries):
        ...
        # if country not in ['ROW']:
            ...
            if np.allclose(df_c[f'{qties_dic[qty]["column_name"]}_{country}'],
                           df_c[f'{qties_dic[qty]["column_name"]}_{country}'].sort_values(ascending=False)):
                df.loc[country,qty] = np.nan
...
for i,rob_check in enumerate(variations_of_robust_checks):
    ...
    df_welf.loc['South',variations_of_robust_checks[rob_check]+' welfare change'] = dyn_sol_pre_cf_fix_north.compute_consumption_equivalent_welfare_for_subset_of_countries(
        p_pre_cf_fix_north,['CHN','BRA','IND','RUS','MEX','ZAF'])['pop_weighted']*100-100
...
def smooth_curve(x, y):
    # Generate new x values with 10 times more points
    x2 = np.logspace(np.log10(min(x)), np.log10(max(x)), len(x) * 10)
    
    # Create a spline of order 3 (cubic spline)
    spline = make_interp_spline(x, y, k=3)
    
    # Generate smoothed y values using the spline
    y2 = spline(x2)
...
for country in p_baseline.countries:
    ...
    for i,rob_check in enumerate(variations_of_robust_checks):
        ...
        #             )
        # else:
        if variations_of_robust_checks[rob_check].startswith('With'):
            ls = '-'
            zorder = 99
        ...
        if variations_of_robust_checks[rob_check].startswith('Baseline'):
            ...
            color='grey'
            # lw=0.5
        ax.plot(smooth_curve(df_welfare['delt'], 
                             df_welfare[country])[0],
                smooth_curve(df_welfare['delt'], 
                             df_welfare[country])[1]*100-100,
                label=variations_of_robust_checks[rob_check],
                color = color,
                zorder=zorder,
                # lw=lw,
    ...
    export_legend(legend,filename = with_entry_costs_path+'legend.pdf')
    # plt.legend(False)
    legend.remove()
    # ax.legend(ncol = 2,loc=[1.02,0.02])
    # plt.title('Robustness check of unilateral patent policy counterfactual for '+countries_names[country])
    for save_format in save_formats:
        plt.savefig(with_entry_costs_path+'unilateral_patent_protection_'+country+'.'+save_format,format=save_format)
    plt.show()
    ...
    for i,rob_check in enumerate(variations_of_robust_checks):
        ...
        if variations_of_robust_checks[rob_check].startswith('Baseline'):
            ...
            color='grey'
            lw=0.5
        ax.plot(smooth_curve(other_countries_welfare['delt'], 
                     other_countries_welfare['other_countries'])[0],
                smooth_curve(other_countries_welfare['delt'], 
                             other_countries_welfare['other_countries'])[1]*100-100,
                label=variations_of_robust_checks[rob_check],
                color = color,
                zorder=zorder,
                lw=lw,
    ...
    for save_format in save_formats:
        plt.savefig(with_entry_costs_path+'unilateral_patent_protection_'+country+'_other_countries.'+save_format,format=save_format)
    plt.show()
...
coop_deltas = pd.read_csv('coop_eq_recaps/deltas.csv',index_col=0).drop_duplicates(['baseline',
                                                     'variation',
                                                     'aggregation_method'],keep='last')

df = coop_deltas.loc[(coop_deltas.baseline == int(baseline))
                        & (coop_deltas.variation.isin(list(variations_of_robust_checks.keys())))
                        & (coop_deltas.aggregation_method=='pop_weighted')
                           ].drop('baseline',axis=1)

dfw = pd.DataFrame(columns=['variation', 'aggregation_method', 'USA', 'EUR', 'JAP', 'CHN', 'BRA',
       'IND', 'CAN', 'KOR', 'RUS', 'MEX', 'ZAF', 'ROW', 'Equal', 'Negishi',
       'growth rate'])

for country in p_baseline.countries:
    df[country][df[country]>1] = 12
    df[country][df[country]<1] = 0.01
...
for i,rob_check in enumerate(variations_of_robust_checks):
    ...
    if rob_check == 'baseline':
        ...
        local_path = \
            results_path+f'baseline_{baseline}_variations/{rob_check}/'
    ...
    if rob_check == 'baseline':
        print('baseline')
        sol, sol_b = fixed_point_solver(p,x0=p.guess,
                                        context = 'counterfactual',
                                cobweb_anim=False,tol =1e-14,
                                accelerate=False,
                                accelerate_when_stable=True,
    ...
    if rob_check == '11.02':
        print('11.02')
        # sol, sol_b = fixed_point_solver_with_entry_costs(p,x0=p.guess,
                                # context = 'counterfactual',
                                ...
                                # damping_post_acceleration=10
                                # )
        sol_b = var_with_entry_costs.var_from_vector(p.guess, p, 'counterfactual')    
    
    sol_b.scale_P(p)
    sol_b.compute_non_solver_quantities(p) 
    
    for coop in ['pop_weighted']:
        
        p.delta[...,1] = df.set_index(['variation','aggregation_method']
                                      ).loc[(variation,coop)
                                            ][p_baseline.countries
                                              ].values.squeeze()
        ...
        sol_c.compute_consumption_equivalent_welfare(p,sol_b)
        sol_c.compute_world_welfare_changes(p,sol_b)
        
        dfw.loc[i] = [variations_of_robust_checks[rob_check],
                      coop
                      ]+sol_c.cons_eq_welfare.tolist()+[
...
df = df[['variation', 'aggregation_method']+p.countries]
dfw = dfw[['variation', 'aggregation_method']+p.countries+['Equal', 'Negishi', 'growth rate']]

df.T.to_csv(with_entry_costs_path+'coop_equilibria.csv')
dfw.T.to_csv(with_entry_costs_path+'coop_equilibria_welfare.csv')
...
nash_deltas = pd.read_csv('nash_eq_recaps/deltas.csv',index_col=0).drop_duplicates(['baseline',
                                                     'variation'],keep='last')
...
dfw = pd.DataFrame(columns=['variation', 'USA', 'EUR', 'JAP', 'CHN', 'BRA',
       'IND', 'CAN', 'KOR', 'RUS', 'MEX', 'ZAF', 'ROW', 'Equal', 'Negishi',
       'growth rate'])
...
for i,rob_check in enumerate(variations_of_robust_checks):
    ...
    p.delta[...,1] = df.set_index('variation'
                                  ).loc[variation
                                        ][p_baseline.countries
                                          ].values.squeeze()
...
df = df[['variation']+p.countries]
dfw = dfw[['variation']+p.countries+['Equal', 'Negishi', 'growth rate']]

df.T.to_csv(with_entry_costs_path+'nash.csv')
dfw.T.to_csv(with_entry_costs_path+'nash_welfare.csv')

#%% pre-TRIPS Counterfactuals

run_path_with_entry_costs = f'calibration_results_matched_economy/baseline_{baseline}_variations/{variation_with_entry_costs}/'

p_baseline_with_entry_costs = parameters()
p_baseline_with_entry_costs.load_run(run_path_with_entry_costs)

sol_baseline_with_entry_costs = var_with_entry_costs.var_from_vector(p_baseline_with_entry_costs.guess, 
                                                                       p_baseline_with_entry_costs, 
                                                                       compute=True, 
                                                                       context = 'counterfactual')
sol_baseline_with_entry_costs.scale_P(p_baseline_with_entry_costs)
sol_baseline_with_entry_costs.compute_non_solver_quantities(p_baseline_with_entry_costs)

p_pre = parameters()
p_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline_pre_trips_variation}_variations/{pre_trips_variation_with_entry_costs}/')
_, sol_pre = fixed_point_solver_with_entry_costs(p_pre,context = 'counterfactual',x0=p_pre.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
...
p_pre_cf = p_baseline_with_entry_costs.copy()
p_pre_cf.delta[...,1] = p_pre.delta[...,1]

_, sol_pre_cf = fixed_point_solver_with_entry_costs(p_pre_cf,context = 'counterfactual',x0=p_pre_cf.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
                        cobweb_qty='phi',
...
sol_pre_cf.scale_P(p_pre_cf)
sol_pre_cf.compute_non_solver_quantities(p_pre_cf)
sol_pre_cf.compute_consumption_equivalent_welfare(p_pre_cf,sol_baseline_with_entry_costs)
sol_pre_cf.compute_world_welfare_changes(p_pre_cf,sol_baseline_with_entry_costs)

p_pre_cf_fix_north = p_baseline_with_entry_costs.copy()
p_pre_cf_fix_north.delta[...,1] = p_pre.delta[...,1]
for country_idx in [[p_baseline.countries.index(c) for c in rich_countries]]:
    p_pre_cf_fix_north.delta[country_idx,1] = p_baseline_with_entry_costs.delta[country_idx,1]

_, sol_pre_cf_fix_north = fixed_point_solver_with_entry_costs(p_pre_cf_fix_north,context = 'counterfactual',x0=p_pre_cf_fix_north.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
                        cobweb_qty='phi',
...
sol_pre_cf_fix_north.scale_P(p_pre_cf_fix_north)
sol_pre_cf_fix_north.compute_non_solver_quantities(p_pre_cf_fix_north)
sol_pre_cf_fix_north.compute_consumption_equivalent_welfare(p_pre_cf_fix_north,sol_baseline_with_entry_costs)
sol_pre_cf_fix_north.compute_world_welfare_changes(p_pre_cf_fix_north,sol_baseline_with_entry_costs)

p_pre_trips_increase_only = p_baseline_with_entry_costs.copy()
p_pre_trips_increase_only.delta[...,1] = np.maximum(p_pre.delta[...,1],p_baseline_with_entry_costs.delta[...,1])

_, sol_pre_trips_increase_only = fixed_point_solver_with_entry_costs(p_pre_trips_increase_only,
                                                                     context = 'counterfactual',
                                                                     x0=p_pre_trips_increase_only.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
                        cobweb_qty='phi',
...
sol_pre_trips_increase_only.scale_P(p_pre_trips_increase_only)
sol_pre_trips_increase_only.compute_non_solver_quantities(p_pre_trips_increase_only)
sol_pre_trips_increase_only.compute_consumption_equivalent_welfare(p_pre_trips_increase_only,sol_baseline_with_entry_costs)
sol_pre_trips_increase_only.compute_world_welfare_changes(p_pre_trips_increase_only,sol_baseline_with_entry_costs)

p_pre_cf_fix_north_and_tariff = p_baseline_with_entry_costs.copy()
p_pre_cf_fix_north_and_tariff.delta[...,1] = p_pre.delta[...,1]
p_pre_cf_fix_north_and_tariff.tariff = p_pre.tariff.copy()
for country_idx in [[p_baseline.countries.index(c) for c in rich_countries]]:
    p_pre_cf_fix_north_and_tariff.delta[country_idx,1] = p_baseline_with_entry_costs.delta[country_idx,1]

_, sol_pre_cf_fix_north_and_tariff = fixed_point_solver_with_entry_costs(p_pre_cf_fix_north_and_tariff,
                                                     context = 'counterfactual',
                        x0=p_pre_cf_fix_north_and_tariff.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
...
sol_pre_cf_fix_north_and_tariff.compute_non_solver_quantities(p_pre_cf_fix_north_and_tariff)
sol_pre_cf_fix_north_and_tariff.compute_consumption_equivalent_welfare(p_pre_cf_fix_north_and_tariff,
                                                                    sol_baseline_with_entry_costs)
sol_pre_cf_fix_north_and_tariff.compute_world_welfare_changes(p_pre_cf_fix_north_and_tariff,
                                                           sol_baseline_with_entry_costs)

p_pre_cf_fix_north_and_tariff_in_pat_sect = p_baseline_with_entry_costs.copy()
p_pre_cf_fix_north_and_tariff_in_pat_sect.delta[...,1] = p_pre.delta[...,1]
p_pre_cf_fix_north_and_tariff_in_pat_sect.tariff[...,1] = p_pre.tariff[...,1]
for country_idx in [[p_baseline.countries.index(c) for c in rich_countries]]:
    p_pre_cf_fix_north_and_tariff_in_pat_sect.delta[country_idx,1] = p_baseline_with_entry_costs.delta[country_idx,1]

_, sol_pre_cf_fix_north_and_tariff_in_pat_sect = fixed_point_solver_with_entry_costs(p_pre_cf_fix_north_and_tariff_in_pat_sect,
                                                     context = 'counterfactual',
                        x0=p_pre_cf_fix_north_and_tariff_in_pat_sect.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
...
sol_pre_cf_fix_north_and_tariff_in_pat_sect.scale_P(p_pre_cf_fix_north_and_tariff_in_pat_sect)
sol_pre_cf_fix_north_and_tariff_in_pat_sect.compute_non_solver_quantities(p_pre_cf_fix_north_and_tariff_in_pat_sect)
sol_pre_cf_fix_north_and_tariff_in_pat_sect.compute_consumption_equivalent_welfare(p_pre_cf_fix_north_and_tariff_in_pat_sect,sol_baseline_with_entry_costs)
sol_pre_cf_fix_north_and_tariff_in_pat_sect.compute_world_welfare_changes(p_pre_cf_fix_north_and_tariff_in_pat_sect,sol_baseline_with_entry_costs)

p_pre_cf_tariff = p_baseline_with_entry_costs.copy()
p_pre_cf_tariff.tariff[...] = p_pre.tariff[...]

_, sol_pre_cf_tariff = fixed_point_solver_with_entry_costs(p_pre_cf_tariff,context = 'counterfactual',
                        x0=p_pre_cf_tariff.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
                        cobweb_qty='phi',
...
sol_pre_cf_tariff.scale_P(p_pre_cf_tariff)
sol_pre_cf_tariff.compute_non_solver_quantities(p_pre_cf_tariff)
sol_pre_cf_tariff.compute_consumption_equivalent_welfare(p_pre_cf_tariff,sol_baseline_with_entry_costs)
sol_pre_cf_tariff.compute_world_welfare_changes(p_pre_cf_tariff,sol_baseline_with_entry_costs)

p_pre_cf_tariff_in_pat_sect = p_baseline_with_entry_costs.copy()
p_pre_cf_tariff_in_pat_sect.tariff[...,1] = p_pre.tariff[...,1]

_, sol_pre_cf_tariff_in_pat_sect = fixed_point_solver_with_entry_costs(p_pre_cf_tariff_in_pat_sect
                        ,context = 'counterfactual',x0=p_pre_cf_tariff_in_pat_sect.guess,
                        cobweb_anim=False,tol =1e-14,
                        accelerate=False,
                        accelerate_when_stable=True,
...
sol_pre_cf_tariff_in_pat_sect.scale_P(p_pre_cf_tariff_in_pat_sect)
sol_pre_cf_tariff_in_pat_sect.compute_non_solver_quantities(p_pre_cf_tariff_in_pat_sect)
sol_pre_cf_tariff_in_pat_sect.compute_consumption_equivalent_welfare(p_pre_cf_tariff_in_pat_sect,sol_baseline_with_entry_costs)
sol_pre_cf_tariff_in_pat_sect.compute_world_welfare_changes(p_pre_cf_tariff_in_pat_sect,sol_baseline_with_entry_costs)
...
df = pd.DataFrame(
    index = pd.Index([modified_countries_names[c] for c in p_baseline_with_entry_costs.countries]+['World\nNegishi','World\nEqual'],
                                       name = 'country')
    )

df['delta_baseline'] = p_baseline_with_entry_costs.delta[...,1].tolist()+[np.nan,np.nan]
df['delta_1992'] = p_pre.delta[...,1].tolist()+[np.nan,np.nan]
df['static_welfare_change'] = sol_pre_cf.cons_eq_welfare.tolist()+[
    sol_pre_cf.cons_eq_negishi_welfare_change,sol_pre_cf.cons_eq_pop_average_welfare_change
    ]
...
for col in ['static_welfare_change',
            'static_welfare_change_fixed_delta_north',
            'static_welfare_change_fixed_delta_north_and_cf_tariff',
            'static_welfare_change_fixed_delta_north_and_cf_tariff_in_pat_sect',
            'static_welfare_change_increase_only',]:
...
df.loc['growth_rate','delta_baseline'] = sol_baseline_with_entry_costs.g
df.loc['growth_rate','static_welfare_change'] = sol_pre_cf.g
df.loc['growth_rate','static_welfare_change_cf_tariff'] = sol_pre_cf_tariff.g
df.loc['growth_rate','static_welfare_change_cf_tariff_in_pat_sect'] = sol_pre_cf_tariff_in_pat_sect.g
df.loc['growth_rate','static_welfare_change_fixed_delta_north'] = sol_pre_cf_fix_north.g
...
df.style.format(precision=5).to_latex(with_entry_costs_path+'pre_trips.tex',
                  caption=caption,
                  **save_to_tex_options
                  )
