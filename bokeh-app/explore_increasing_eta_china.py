...
eta_time_partial = np.concatenate([pd.read_csv(f'calibration_results_matched_economy/baseline_{baseline}_variations/11.{i}/eta.csv'
                      ...
                      for i in range(29)],axis=1)   
delta_coop = pd.read_csv('coop_eq_recaps/deltas.csv',index_col=0).drop_duplicates(['baseline',
                                                     'variation',
...
#                                  & (delta_coop.variation.str.startswith('20'))].sort_values('variation',key=sort_key)['CHN'].values
...
# ax2.plot([y for y in range(1990,1990+50)][-25:],delta_coop_time,
#          label='Delta in\ncoop eq',color=sns.color_palette()[1])
ax2.set_yscale('log')
