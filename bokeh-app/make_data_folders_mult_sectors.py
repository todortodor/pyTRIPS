...
country_sector_moments_data = pd.read_csv(
    legacy_data_path+'anberd.csv')
...
for config_dic in config_dics:
    ...
    nbr_of_sectors = config_dic['nbr_sectors']
    path = f'data/data_{nbr_of_countries}_countries_{nbr_of_sectors}_sectors_{year}/'
    dropbox_path = f'/Users/slepot/Dropbox/TRIPS/Calibration data/calibration_data_folders/data_{nbr_of_countries}_countries_{nbr_of_sectors}_sectors_{year}/'
    ...
    country_sector_moments_data = pd.read_csv(
        legacy_data_path+'anberd.csv')
    country_sector_moments_data = country_sector_moments_data[
        ['REF_AREA','CRITERIA','ACTIVITY',
               'UNIT_MEASURE', 'PRICE_BASE',
               'TIME_PERIOD', 'OBS_VALUE']
        ]
    
    country_sector_moments_data = country_sector_moments_data.loc[
        (country_sector_moments_data['CRITERIA'] == 'MA') &
        (country_sector_moments_data['UNIT_MEASURE'] == 'USD_PPP') &
        (country_sector_moments_data['PRICE_BASE'] == 'V') &
        (country_sector_moments_data['TIME_PERIOD'] == year)
        ]
    
    country_sector_moments_data = country_sector_moments_data[
        ['REF_AREA','ACTIVITY','OBS_VALUE']
        ]
    
    country_sector_moments_data = country_sector_moments_data.rename(
    ...
    country_sector_moments_data['country'] = country_sector_moments_data['country'].map(
        crosswalk_countries['country_code'])
    country_sector_moments_data = country_sector_moments_data.dropna(subset=['country'])
    country_sector_moments_data = country_sector_moments_data.groupby(['sector','country']).sum().sort_index()
    
    country_sector_moments = pd.DataFrame(
        index = country_sector_moments_data.index.get_level_values(1).drop_duplicates()
        ).sort_index().rename_axis(['country'])
    
    country_sector_moments['RD ratio pharma'] = country_sector_moments_data.loc['C21']['RD'] / country_sector_moments_data.loc['_T']['RD']
    country_sector_moments['RD ratio chemicals'] = country_sector_moments_data.loc['C20']['RD'] / country_sector_moments_data.loc['_T']['RD']
    
    if write:
        country_sector_moments.to_csv(path+'country_sector_moments.csv')
        country_sector_moments.to_csv(dropbox_path+'country_sector_moments.csv')
    
    # get patent flows

    pflows = pd.read_csv(
        pflows_path+f'{nbr_of_countries}_countries_{nbr_of_sectors}_sectors/flows_{nbr_of_countries}_countries_{year}.csv', index_col=[0, 1, 2]
    )
    ...
    codes_similar_to_india = [4, 9, 5, 12]
    
    for origin in indian_flows.index:
        for sector in range(1,nbr_of_sectors):
            pflows.loc[(origin,6.0,sector),'patent flows'] = indian_flows.loc[origin,'patent flows'] \
                * pflows.loc[(origin,codes_similar_to_india,sector),'patent flows'].sum() \
                    / pflows.loc[(origin,codes_similar_to_india),'patent flows'].sum()  
    ...
    for origin_code in range(1, nbr_of_countries+1):
        for destination_code in range(1, nbr_of_countries+1):
            for sector in range(1,nbr_of_sectors):
                if (origin_code, destination_code,sector) not in pflows.index or pflows.loc[(origin_code, destination_code,sector)].isna().any():
                    pflows.loc[(origin_code, destination_code,sector),
                                'patent flows'] = 0
                
    for origin_code in range(1, nbr_of_countries+1):
        for destination_code in range(1, nbr_of_countries+1):
            for sector in range(1,nbr_of_sectors):
                if pflows.loc[(origin_code, destination_code,sector),
                            'patent flows'] == 0:
                    pflows.loc[(origin_code, destination_code,sector),
                                'patent flows'] = pflows.loc[
                                    pflows['patent flows'] > 0, 'patent flows'
                                                ...
                                                )*pflows.xs(sector,level=2)['patent flows'].sum(
    ...
    scalar_moments.loc['UUPCOST', 'value'] = pflows.loc[(1, 1), 'patent flows'].sum()*final_pat_fees.loc[1, 'fee']\
        * gdp_deflator.loc['USA', str(year)]/gdp_deflator.loc['USA', '2005']/1e12
    
    scalar_moments.loc['JUPCOST', 'value'] = pflows.loc[(1, 3), 'patent flows'].sum()*final_pat_fees.loc[1, 'fee']\
        * gdp_deflator.loc['USA', str(year)]/gdp_deflator.loc['USA', '2005']/1e12
    ...
    scalar_moments.loc['TOPHARMACHEM', 'value'] = 0.07944382
    scalar_moments.loc['TOPATENT', 'value'] = 0.070641617
    scalar_moments.loc['AGGAVMARKUP', 'value'] = 1.06294875
    ...
    scalar_moments.loc['KMPHARMACHEM', 'value'] = scalar_moments.loc['KM', 'value'] * 1.90567074716
    scalar_moments.loc['KMPATENT', 'value'] = scalar_moments.loc['KM', 'value'] * 0.91324916669 
    scalar_moments.loc['TECHEM', 'value'] = 3.13
    scalar_moments.loc['TEPHARMA', 'value'] = 3.13
    scalar_moments.loc['PROBINNOVENT', 'value'] = 0.7
    scalar_moments.loc['SHAREEXPMON', 'value'] = 0.3
