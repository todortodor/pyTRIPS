...
class var_with_fdi:
    ...
    def compute_case_indicator(self, p):
        """
        Case 2: all off-diagonal patenting sector pairs.
        We always treat n!=i pairs as Case 2 during the fixed-point iteration.
        The gamma fractions (gamma_PF, gamma_NPF) naturally go to zero when
        FDI is not profitable (large a_NPF_NPO), so there is no need to
        hard-gate on the profit comparison — that comparison is only valid
        at the fixed point, not during iteration, and creates a self-fulfilling
        zero-FDI equilibrium if used as a gate.
        """
    ...
    def compute_labor_research(self, p):
        ...
        # ── Case 2 terms: IntVNorigP + IntVorigP (eqs 50-54) ─────────────────
        IntV_c2 = np.zeros((p.N, p.N, p.S-1))
        for s_idx in range(p.S-1):
            s = s_idx + 1  # full sector index
            for i in range(p.N):
                psi_o = float(self.psi_o_star[i, s])
                for n in range(p.N):
                    if n == i or not self.case2[n, i, s]:
                        continue
                    a_bar = float(self.a_NPF_NPO[n,i,s])   # a^{NPF_NPO}
                    a_hat = float(self.a_PF_NPO[n,i,s])    # a^{PF_NPO}
                    a_chk = float(self.a_PF_PO[n,i,s])     # a^{PF,PO}
                    psi_O = float(self.psi_star_O[n,i,s])  # psi^{*,O}
                    psi_F = float(self.psi_star_F[n,i,s])  # psi^{*,F}
                    V_NPO = float(self.V_NP[n,i,s])
                    V_NPF = float(self.V_NP_F[n,i,s])
                    V_PO  = float(self.V_P[n,i,s])
                    V_PF  = float(self.V_P_F[n,i,s])
                    w_n   = float(self.w[n])
                    w_i   = float(self.w[i])
                    a_nis = float(self.a[n,i,s])
                    w_n_a = w_n * a_nis
                    w_n_fe_h = w_n * float(p.r_hjort[n]) * float(p.fe[s])
                    ...
                    if a_bar < 1:
                        ...
                        min_a_bar_psi_o = min(a_bar, psi_o)
                        IntVN = (k/(k-1)*(1 - min_a_bar_psi_o**(1-k))*V_NPO
                                 + V_NPF*k/(k-1)*(min_a_bar_psi_o**(1-k) - psi_o**(1-k))
                                 - w_n_a*d/(d+1)*(min_a_bar_psi_o**k - psi_o**k)
                                 + k*a_bar**d*(V_NPF - V_NPO)/(d*k+1)
                                   *(min_a_bar_psi_o**(d*k+1) - 1))
 
                    # ── IntVorigP_A (eq 51) ───────────────────────────────────
                    if psi_o < psi_O:
                        IntVA = k/(k-1)*(psi_o**(1-k)*V_NPO
                                         + (psi_O**(1-k) - psi_o**(1-k))*V_PO
                                         - psi_O**k * w_n_fe_h)
                    else:
                        IntVA = k/(k-1)*(psi_o**(1-k)*(V_PO - w_n_fe_h)
                                         - psi_o**k * w_n_fe_h)
                    ...
                    if psi_o < psi_F:
                        if a_bar < psi_o:
                            IntVB = (k/(k-1)*(V_NPF - w_n_a*d/(d+1))
                                     * (psi_o**k - psi_F**k))
                        elif psi_o <= a_bar < psi_F:
                            IntVB = (k/(k-1)*V_NPO*(psi_o**(1-k) - a_bar**(1-k))
                                     + k*a_bar**d*(V_NPF - V_NPO)/(d*k+1)
                                       *(a_bar**(d*k+1) - psi_o**(d*k+1))
                                     + k/(k-1)*(V_NPF - w_n_a*d/(d+1))
                                       *(a_bar**k - psi_F**k))
 
                    # ── IntVorigP_C (eq 53) ───────────────────────────────────
                    IntVC = 0.0
                    if psi_F <= psi_o < psi_O:
                        psi_bar = float(self.psi_bar_NPO_PF[n,i,s])
                        if np.isfinite(psi_bar) and psi_bar > 0 and a_nis > 0:
                            ratio = (p.r_hjort[n]*p.fe[s]/a_nis)**d
                            t1 = max(0.0, min(1.0, 1 - psi_bar/max(psi_F, psi_o)))
                            t2 = max(0.0, min(1.0, 1 - psi_bar/min(a_hat, psi_O)))
                            from mpmath import betainc as _bi
                            B1C = float(_bi(d+1, d+k-1, t1, t2, regularized=False).real)
                            B2C = float(_bi(d+2, d+k-1, t1, t2, regularized=False).real)
                            B3C = float(_bi(d+1, d+k-1, t1, t2, regularized=False).real)
                            IntVC = (k*ratio/psi_bar**k
                                     * (V_NPO*B1C
                                        - k*w_n_fe_h/a_nis/(d+1)*B2C
                                        + (V_PF - w_n_fe_h)*B3C))
                    ...
                    if psi_o >= psi_O:
                        if a_chk <= psi_o:
                            IntVD = (k/(k-1)*psi_o**(1-k)*(V_PF - w_n_fe_h)
                                     * (1 - psi_o**k * w_n_a*d/(d+1)))
                        else:
                            IntVD = (k/(k-1)*(V_PO - w_n_fe_h)
                                     * (psi_o**(1-k) - a_chk**(1-k))
                                     + k*a_chk**d*(V_PF - V_PO)/(d*k+1)
                                       *(a_chk**(d*k+1) - psi_o**(d*k+1))
                                     + k/(k-1)*a_chk**(1-k)*(V_PF - w_n_fe_h)
                                       *(1 - a_chk**k * w_n_a*d/(d+1)))
 
                    IntV_c2[n, i, s_idx] = IntVN + IntVA + IntVB + IntVC + IntVD
 
        # IntV_c2 is (n,i,s-1); sum over n to get (i,s-1) contribution
        IntV_sum = np.where(self.case2[...,1:], IntV_c2, 0.0).sum(axis=0)
 
        # ── Case 1 base term from eq (49): k/(k-1)*pi^w/G + psi_mO^{-k}*... ─
        # The Case 1 contribution for n=i pairs (always Case 1)
        # plus off-diagonal Case 1 pairs — already in base_sum
        # Add the Case 1 diagonal (n=i) term explicitly
        diag_term = np.zeros((p.N, p.S-1))
        for s_idx in range(p.S-1):
            s = s_idx + 1
            for i in range(p.N):
                # n=i is always Case 1 (diagonal)
                n = i
                psi_mO = float(self.psi_m_star_O[n,i,s])
                psi_C_O = float(self.psi_star_O[n,i,s]) if float(self.psi_star_O[n,i,s]) > 0 else 1.0
                diag_term[i,s_idx] = (
                    k/(k-1)*float(self.profit[n,i,s])/float(self.G[s])
                    + psi_mO**(-k)*float(self.w[n])*float(p.r_hjort[n])*float(p.fe[s])
                      /float(self.w[i])*(k/(k-1)*psi_mO/psi_C_O - 1))
 
        l_R=np.zeros((p.N,p.S))
        # eq (49): sum over n of [base Case1 + diag + IntV_c2] - psi_o^{-k}*fo*h
        total = (base_sum + diag_term + IntV_sum
...
#     #         & self.off_diag_mask[...,1:]
#     #     )
#     #     # If pi_F is uninitialised (all equal to profit, no Case 2 triggered yet),
#     #     # treat all off-diagonal patenting pairs as Case 2 to escape the zero equilibrium.
#     #     if not profit_comparison.any():
#     #         self.case2[..., 1:] = self.off_diag_mask[..., 1:]
#     #     else:
#     #         self.case2[..., 1:] = profit_comparison
...
#         x0 = np.where(np.isfinite(x0) & (x0 >= 1), x0, 1.0)
#         roots = root(func_to_solve, x0=x0, tol=1e-15)
#         self.psi_o_star[:,1:] = roots.x.reshape(p.N, p.S-1)
...
#             Pi^{w,F}_{nis} = (1/w_n) * (sigma/(sigma-1))^{1-sigma} * Phi_nn^{(sigma-1)/theta}
#                              / D_ns  * P_ns * C_ns
#         where D_ns is the common denominator from compute_sectoral_prices.
#         """
#         pi_F = np.zeros((p.N, p.N, p.S))
    
#         power = (p.sigma - 1) / p.theta   # (S,)
#         markup = (p.sigma / (p.sigma - 1))**(1 - p.sigma)  # (S,)
    
#         phi_nn = np.einsum('nns->ns', self.phi)   # (N,S)
    
#         # Numerator: markup^s * Phi_nn^{(sigma-1)/theta}
#         numer = markup[None, 1:] * phi_nn[:, 1:]**power[None, 1:]  # (N,S-1)
    
#         # Denominator D_ns: same as in compute_sectoral_prices
#         A_exp = markup[None, 1:] * (
#             self.PSI_M_O[..., 1:] * self.phi[..., 1:]**power[None, None, 1:]
#         ).sum(axis=1)   # (N,S-1)
...
#         B_cd = self.PSI_CD[:, 1:] * (
#             self.phi[..., 1:]**p.theta[None, None, 1:]
#         ).sum(axis=1)**((p.sigma - 1) / p.theta)[None, 1:]   # (N,S-1)
    
#         D = A_exp + A_fdi + B_cd   # (N,S-1)
    
#         # P_n^s * C_n^s = beta_s * Z_n / P_n  (sectoral nominal consumption share)
#         # Since P_ns/P_n = (numer/D)^{1/(1-sigma)} and X^{M,F} uses P_ns,
#         # we use: P_ns * C_ns = beta_s * Z_n  (from CES demand)
#         PsCs = p.beta[None, 1:] * self.Z[:, None]   # (N,S-1)
    
#         # pi_F_{nis} = (1/w_n) * numer_ns / D_ns * PsCs_ns
#         # broadcast over origin i: same value for all i given n,s
#         pi_F[..., 1:] = np.where(
#             self.PSI_M_F[..., 1:] > 0,
#             (numer / D * PsCs)[:, None, :] / self.w[:, None, None],
#             0.0)
...
class moments:
    ...
    def load_data(self,data_path = None,dir_path=None):
        ...
        for i_dest, dest in enumerate(self.countries):
            for i_orig, orig in enumerate(self.countries):
                try:
                    fdi_matrix[i_dest, i_orig] = self.fdi_flows.loc[(dest, orig), 'fdi_flow']
                except KeyError:
        ...
        self.FDI_FLOW_N_target = np.where(denom > 0,
                                          (fdi_sum_n / self.unit) / denom,
                                          np.nan)
    ...
    def compute_FDI_FLOW_N(self, var, p):
        """
        Point 15: ratio of total affiliate sales into n over domestic absorption
        minus affiliate sales.
        FDI_FLOW_N_n = sum_i X^{M,F}_{ni} / (X_nn - sum_i X^{M,F}_{ni})
        """
        X_M_F_sum_n = var.X_M_F[:, :, 1:].sum(axis=(1, 2))   # (N,) total FDI received by n
        X_nn = np.einsum('nns->n', var.X[:, :, 1:2]).squeeze() # domestic absorption sector 1
        denom = X_nn - X_M_F_sum_n
        self.FDI_FLOW_N = np.where(denom > 0, X_M_F_sum_n / denom, np.nan)
    ...
    def compute_FDI_ELAST(self, var, p):
        ...
        if theta_base > 0 and np.isfinite(theta_pert):
