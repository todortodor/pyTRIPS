...
for save_path in ['deriv_welfare_to_patent_protec_cf/tau_pat_sector/',
                  'deriv_welfare_to_patent_protec_cf/nu/',
                  'deriv_welfare_to_patent_protec_cf/tariff_pat_sector/'
                  ]:
    try:
        os.mkdir(save_path)
    except:
        pass
    try:
        os.mkdir(save_path+f'baseline_{baseline}/')
    except:
        pass
...
def process_country(args):
    index,qty,factor,folder_name,sol_c,p,country,dynamics = args
    try:
        if dynamics:
            df = pd.read_csv(
                f'deriv_welfare_to_patent_protec_cf/{folder_name}/baseline_{baseline}/dyn_{country}.csv',
                index_col = 0)
        else:
            df = pd.read_csv(
                f'deriv_welfare_to_patent_protec_cf/{folder_name}/baseline_{baseline}/{country}.csv',
                index_col = 0)
    except:
        df = pd.DataFrame(columns = [qty]+p_baseline.countries
                                              +['pop_weighted','negishi'])
    deriv_welfare = compute_deriv_welfare_to_patent_protec(sol_c,p,
                                            country=country,
                                            dynamics=dynamics)
    df.loc[index] = [factor]+deriv_welfare
    print(country,df.T)
    if dynamics:
        df.to_csv(
            f'deriv_welfare_to_patent_protec_cf/{folder_name}/baseline_{baseline}/dyn_{country}.csv')
...
if __name__ == '__main__':
    ...
    for t,tariff_factor in enumerate(np.linspace(0,10,76)):
        print(tariff_factor)
        p = p_baseline.copy()
