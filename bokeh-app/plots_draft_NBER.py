...
variation_with_pre_TRIPS_tau = '10.1'
...
# for save_format in save_formats:
#     plt.savefig(save_path+'international_pat_families_by_origin.'+save_format,format=save_format)
...
df.style.format(precision=5).to_latex(save_path+'uniformization_to_delta_us_table.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(save_path+'uniformization_to_delta_us_table.csv',float_format='%.5f')
...
for c in p_baseline.countries+['World','Uniform_delta','Upper_uniform_delta',
                               'trade_cost_eq_trips_all_countries_pat_sectors']:
...
write_calibration_results(save_path+'pre_TRIPS_partial_calibration',p_pre,m_pre,sol_pre,commentary = '')
...
#%% trade cost equivalent of TRIPS
...
exercises = ['trade_cost_eq_trips_all_countries_all_sectors','trade_cost_eq_trips_all_countries_pat_sectors'
                  ,'dyn_trade_cost_eq_trips_all_countries_all_sectors','dyn_trade_cost_eq_trips_all_countries_pat_sectors']
countries = ['CHN','IND','RUS']

df = pd.DataFrame(columns = exercises,
                  index = [countries_names[c] for c in countries])

for exercise in exercises:
# for exercise in ['trade_cost_eq_trips_all_countries_all_sectors','trade_cost_eq_trips_all_countries_pat_sectors']:
    if variation == 'baseline':
    ...
    df_welfare = pd.read_csv(local_path+exercise+'.csv')
    
    df_welfare.plot(x='delt',y=p_baseline.countries)# plt.xlim(0.95,1.05); plt.ylim(0.95,1.05)
    
    for country in ['CHN','IND','RUS']:
        df.loc[countries_names[country],exercise] = find_zeros(df_welfare,country)
        # print(country,exercise,find_zeros(df_welfare,country))

caption = 'Trade cost equivalent of TRIPS'

df.style.format(precision=5).to_latex(save_path+'trade_cost_eq_trips.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(save_path+'trade_cost_eq_trips.csv',float_format='%.5f')
...
for country in countries:
    ...
    # for exercise in ['trade_cost_eq_trips_all_countries_all_sectors','trade_cost_eq_trips_all_countries_pat_sectors']:
        ...
        # df_welfare.plot(x='delt',y=p_baseline.countries);# plt.xlim(1.08,1.12); plt.ylim(0.95,1.05)
...
#     if data != 'delt':
#         ax.plot(data_for_plot['delt'],data_for_plot[data],label=countries_names[data])
# # plt.xlim(1.08,1.12); 
# plt.legend()
# plt.ylim(0.98,1.02)
# plt.xlim(0.9,1.1)
...
df.style.format(precision=5).to_latex(save_path+'trade_cost_eq_trips_exp_imp_pat_sect.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(save_path+'trade_cost_eq_trips_exp_imp_pat_sect.csv',float_format='%.5f')

#%% Old exercises not relevant anymore

# #%% Nash table of calibration with 1992 trade costs

# all_nashes = pd.read_csv('nash_eq_recaps/deltas.csv')
# all_nashes = all_nashes.drop_duplicates(['baseline','variation'],keep='last')

# run_nash = all_nashes.loc[(all_nashes.baseline == int(baseline)) 
#                           & (all_nashes.variation == variation_with_pre_TRIPS_tau)]

# p_pre = parameters()
# p_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline_pre_trips_variation}_variations/{variation_with_pre_TRIPS_tau}/')
# _, sol_pre = fixed_point_solver(p_pre,context = 'counterfactual',x0=p_pre.guess,
#                         cobweb_anim=False,tol =1e-14,
#                         accelerate=False,
#                         accelerate_when_stable=True,
...
# m_pre = moments()
# m_pre.load_run(f'calibration_results_matched_economy/baseline_{baseline_pre_trips_variation}_variations/{variation_with_pre_TRIPS_tau}/')
...
# sol_nash.scale_P(p_nash)
# sol_nash.compute_non_solver_quantities(p_nash)
# sol_nash.compute_consumption_equivalent_welfare(p_nash,sol_pre)
# sol_nash.compute_world_welfare_changes(p_nash,sol_pre)
...
# caption = 'Nash equilibrium with 1992 trade costs'

# df.style.format(precision=5).to_latex(save_path+'Nash_table_with_1992_trade_costs.tex',
#                   caption=caption,
#                   **save_to_tex_options
#                   )

# df.to_csv(save_path+'Nash_table_with_1992_trade_costs.csv',float_format='%.5f')

# write_calibration_results(save_path+'Nash_with_1992_trade_costs',p_nash,m_nash,sol_nash,commentary = '')

# #%% Nash table with transitional dynamics with 1992 trade costs

# all_nashes = pd.read_csv('nash_eq_recaps/dyn_deltas.csv')
# all_nashes = all_nashes.drop_duplicates(['baseline','variation'],keep='last')

# run_nash= all_nashes.loc[(all_nashes.baseline == int(baseline)) 
#                          & (all_nashes.variation.astype(str) == variation_with_pre_TRIPS_tau)]
...
# df.style.format(precision=5).to_latex(save_path+'dyn_Nash_table_with_1992_trade_costs.tex',
#                   caption=caption,
#                   **save_to_tex_options
#                   )

# df.to_csv(save_path+'dyn_Nash_table_with_1992_trade_costs.csv',float_format='%.5f')

# write_calibration_results(save_path+'dyn_Nash_with_1992_trade_costs',p_nash,m_nash,dyn_sol_nash.sol_fin,commentary = '')

# #%% Coop equal weights table with 1992 trade costs
...
# run_coop_equal= all_coop_equales.loc[(all_coop_equales.baseline == int(baseline))
#                                      & (all_coop_equales.variation == variation_with_pre_TRIPS_tau)
#                                      & (all_coop_equales.aggregation_method == 'pop_weighted')]
...
# sol_coop_equal.scale_P(p_coop_equal)
# sol_coop_equal.compute_non_solver_quantities(p_coop_equal)
# sol_coop_equal.compute_consumption_equivalent_welfare(p_coop_equal,sol_pre)
# sol_coop_equal.compute_world_welfare_changes(p_coop_equal,sol_pre)
...
# caption = 'Cooperative equilibrium with population weights with 1992 trade costs'

# df.style.format(precision=5).to_latex(save_path+'Coop_population_weights_table_with_1992_trade_costs.tex',
#                   caption=caption,
#                   **save_to_tex_options
#                   )

# df.to_csv(save_path+'Coop_population_weights_table_with_1992_trade_costs.csv',float_format='%.5f')

# write_calibration_results(save_path+'Coop_population_weights_with_1992_trade_costs',p_coop_equal,m_coop_equal,sol_coop_equal,commentary = '')


# #%% Coop negishi weights table with 1992 trade costs
...
# run_coop_negishi= all_coop_negishies.loc[(all_coop_negishies.baseline == int(baseline))
#                                      & (all_coop_negishies.variation == variation_with_pre_TRIPS_tau)
#                                      & (all_coop_negishies.aggregation_method == 'negishi')]
...
# sol_coop_negishi.scale_P(p_coop_negishi)
# sol_coop_negishi.compute_non_solver_quantities(p_coop_negishi)
# sol_coop_negishi.compute_consumption_equivalent_welfare(p_coop_negishi,sol_pre)
# sol_coop_negishi.compute_world_welfare_changes(p_coop_negishi,sol_pre)
...
# caption = 'Cooperative equilibrium with Negishi weights with 1992 trade costs'

# df.style.format(precision=5).to_latex(save_path+'Coop_negishi_weights_table_with_1992_trade_costs.tex',
#                   caption=caption,
#                   **save_to_tex_options
#                   )

# df.to_csv(save_path+'Coop_negishi_weights_table_with_1992_trade_costs.csv',float_format='%.5f')

# write_calibration_results(save_path+'Coop_negishi_weights_table_with_1992_trade_costs',p_coop_negishi,m_coop_negishi,sol_coop_negishi,commentary = '')

#%% Coop and Nash equilibria with doubled trade costs in patenting sector

#%% Nash table of calibration with doubled trade costs in patenting sector

all_nashes = pd.read_csv('nash_eq_recaps/deltas.csv')
all_nashes = all_nashes.drop_duplicates(['baseline','variation'],keep='last')

run_nash = all_nashes.loc[(all_nashes.baseline == int(baseline)) 
                          & (all_nashes.variation == variation_with_doubled_tau_in_pat_sect)]
...
df.style.format(precision=5).to_latex(doubled_trade_costs_path+'Nash_table_with_doubled_trade_costs_in_pat_sect.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(doubled_trade_costs_path+'Nash_table_with_doubled_trade_costs_in_pat_sect.csv',float_format='%.5f')

write_calibration_results(doubled_trade_costs_path+'Nash_with_doubled_trade_costs_in_pat_sect',p_nash,m_nash,sol_nash,commentary = '')
...
#%% Coop equal weights table with doubled trade costs in patenting sector
...
caption = 'Cooperative equilibrium with doubled trade costs in the patenting sector'

df.style.format(precision=5).to_latex(doubled_trade_costs_path+'Coop_population_weights_table_with_doubled_trade_costs_in_pat_sect.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(doubled_trade_costs_path+'Coop_population_weights_table_with_doubled_trade_costs_in_pat_sect.csv',float_format='%.5f')

write_calibration_results(doubled_trade_costs_path+'Coop_population_weights_with_doubled_trade_costs_in_pat_sect',p_coop_equal,m_coop_equal,sol_coop_equal,commentary = '')


#%% Coop negishi weights table with doubled trade costs in patenting sector
...
run_coop_negishi= all_coop_negishies.loc[(all_coop_negishies.baseline == int(baseline))
                                     & (all_coop_negishies.variation == variation_with_doubled_tau_in_pat_sect)
                                     & (all_coop_negishies.aggregation_method == 'negishi')]
...
caption = 'Cooperative equilibrium with Negishi weights with doubled trade costs in the patenting sector'

df.style.format(precision=5).to_latex(doubled_trade_costs_path+'Coop_negishi_weights_table_with_doubled_trade_costs_in_pat_sect.tex',
                  caption=caption,
                  **save_to_tex_options
                  )

df.to_csv(doubled_trade_costs_path+'Coop_negishi_weights_table_with_doubled_trade_costs_in_pat_sect.csv',float_format='%.5f')

write_calibration_results(doubled_trade_costs_path+'Coop_negishi_weights_table_with_doubled_trade_costs_in_pat_sect',p_coop_negishi,m_coop_negishi,sol_coop_negishi,commentary = '')
